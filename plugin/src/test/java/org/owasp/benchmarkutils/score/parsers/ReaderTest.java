...
public class ReaderTest {
    ...
    private final Reader reader = new TestableReader();
    ...
    @ParameterizedTest
    @MethodSource(
            "org.owasp.benchmarkutils.score.parsers.ReaderTest#extractTimeFromFilenameTestValues")
    void extractsTimeFromFilename(String filename, long expectedTime) throws IOException {
        ResultFile resultFile = new ResultFile(File.createTempFile(filename, ""));

        assertEquals(expectedTime, reader.extractTimeFromFilename(resultFile));
    }

    private static Stream<Arguments> extractTimeFromFilenameTestValues() {
        return Stream.of(
                Arguments.of("resultfile", -1),
                Arguments.of("resultfile.xxx", -1),
                Arguments.of("resultfile-.xxx", -1),
                Arguments.of("resultfile-1234.xxx", 1234000),
                Arguments.of("resultfile-1234-4321.xxx", 4321000));
    }

    static class TestableReader extends Reader {
