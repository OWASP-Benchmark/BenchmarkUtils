...
public class CrashtestReader extends Reader {
    ...
    private TestCaseResult parseCrashtestIssue(Node testcase) throws URISyntaxException {
        ...
        if (cwe != -1) {
            ...
            if (startOfUrl == -1) {
                ...
                if (startOfUrl == -1) {
                    // And again for: on site vespiary.de:8443/benchmark/xss-02/BenchmarkTest01181.
                    // The ...
                    URLStart = "on site ";
                    startOfUrl = message.indexOf(URLStart);
                    if (startOfUrl == -1) {
                        if (!message.equals("Failed")) {
                            System.out.println(
                                    "Couldn't find URL in Crashtest finding message: " + message);
                        }
                        return null;
                    }
                }
    ...
    /**
     * Map Crashtest rule to CWE from testcase: e.g., <testcase classname="xxe.crashtest.cloud"
